...
public sealed class WindowsScreenSaverSync : IScreenSaverSync
{
    ...
    public void Suppress() => _ = NativeMethods.SetThreadExecutionState(NativeMethods.ES_DISPLAY_REQUIRED);
    public void Restore() { }

    private static async Task PollAsync(Action onActivated, Action onDeactivated, CancellationToken ct)
    {
        ...
        try
        {
            while (await timer.WaitForNextTickAsync(ct))
            {
                ...
                if (isRunning && !wasRunning) onActivated();
                else if (!isRunning && wasRunning) onDeactivated();
