        ...
        if (nCode >= 0)
        {
            ...
            if (msg == NativeMethods.WM_MOUSEMOVE)
            {
                ...
                if (IsOnVirtualScreen)
                {
                    ...
                    // being handled and a warp actually landing, and with recentring now needed
                    // only rarely (RecenterIfDrifted's dead zone) that gap was wide enough to make
                    // reported movement a fraction of the real thing. _lastWarpX/Y double as the
