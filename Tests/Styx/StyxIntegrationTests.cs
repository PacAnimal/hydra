...
public class StyxIntegrationTests
{
    ...
    public async Task TwoHydraClients_MessageIntegrity()
    {
        ...
        await using var sender = new HydraTestClient(_factory!, new HydraConfig { NetworkConfig = cfg, HostName = "sender" });
        await using var receiver = new HydraTestClient(_factory!, new HydraConfig { NetworkConfig = cfg, HostName = "receiver" });
    ...
    public async Task TwoHydraClients_BidirectionalExchange()
    {
        ...
        await using var alpha = new HydraTestClient(_factory!, new HydraConfig { NetworkConfig = cfg, HostName = "alpha" });
        await using var beta = new HydraTestClient(_factory!, new HydraConfig { NetworkConfig = cfg, HostName = "beta" });
    ...
    public async Task NetworkIsolation_DifferentNetworks_CannotCommunicate()
    {
        ...
        await using var senderA = new HydraTestClient(_factory!, new HydraConfig { NetworkConfig = cfgA, HostName = "sender" });
        await using var receiverA = new HydraTestClient(_factory!, new HydraConfig { NetworkConfig = cfgA, HostName = "receiver" });
        await using var clientB = new HydraTestClient(_factory!, new HydraConfig { NetworkConfig = cfgB, HostName = "sender" });
    ...
    public async Task DuplicateHostname_OldConnectionKicked()
    {
        ...
        await using var first = new HydraTestClient(_factory!, new HydraConfig { NetworkConfig = cfg, HostName = "duplicate" });
        await first.StartAsync(CancellationToken.None);
        await first.WaitForReady();

        await using var second = new HydraTestClient(_factory!, new HydraConfig { NetworkConfig = cfg, HostName = "duplicate" });
    ...
    public async Task PeersList_UpdatesOnConnectAndDisconnect()
    {
        ...
        await using var clientA = new HydraTestClient(_factory!, new HydraConfig { NetworkConfig = cfg, HostName = "host-a" });
        ...
        var clientB = new HydraTestClient(_factory!, new HydraConfig { NetworkConfig = cfg, HostName = "host-b" });
        await clientB.StartAsync(CancellationToken.None);
        await clientB.WaitForReady();
    ...
    public async Task Encryption_RecoversAfterPeerReconnect()
    {
        ...
        var sender1 = new HydraTestClient(_factory!, new HydraConfig { NetworkConfig = cfg, HostName = "sender" });
        ...
        var sender2 = new HydraTestClient(_factory!, new HydraConfig { NetworkConfig = cfg, HostName = "sender" });
    ...
    public async Task SendToUnknownHost_IsIgnored()
    {
        ...
        await using var client = new HydraTestClient(_factory!, new HydraConfig { NetworkConfig = cfg, HostName = "solo" });
